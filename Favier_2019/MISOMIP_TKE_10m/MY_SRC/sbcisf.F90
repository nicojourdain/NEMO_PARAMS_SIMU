MODULE sbcisf
   ...
   PUBLIC   sbc_isf, sbc_isf_div, sbc_isf_alloc  ! routine called in sbcmod and divhor
   ...
   !!misomip+ REAL(wp), PUBLIC, SAVE ::   rcpi   = 2000.0_wp     ! phycst ?
   !!misomip+ REAL(wp), PUBLIC, SAVE ::   rkappa = 1.54e-6_wp    ! phycst ?
   !!misomip+ REAL(wp), PUBLIC, SAVE ::   rhoisf = 920.0_wp      ! phycst ?
   REAL(wp), PUBLIC, SAVE ::   rcpi    = 0._wp       !!misomip+
   REAL(wp), PUBLIC, SAVE ::   rkappa  = 0._wp       !!misomip+
   REAL(wp), PUBLIC, SAVE ::   rhoisf  = 918.0_wp    !!misomip+
...
!: Variable used in fldread to read the forcing file (nn_isf == 4 .OR. nn_isf == 3)
   ...
   !! $Id: sbcisf.F90 6140 2015-12-21 11:35:23Z timgraham $
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_isf in configuration namelist', lwp )
  ...
  SUBROUTINE sbc_isf_cav( kt )
   ...
   SUBROUTINE sbc_isf_gammats(pgt, pgs, pqhisf, pqwisf )
      ...
      REAL(wp), PARAMETER :: utide = 1.e-2      ! tidal RMS velocity (ISOMIP)
      ...
      CASE ( 1 ) ! gamma is assume to be proportional to u*
         ...
         !! compute ustar (eq. 24)
         zustar(:,:) = SQRT( rn_tfri2 * (utbl(:,:) * utbl(:,:) + vtbl(:,:) * vtbl(:,:) + utide * utide) )
