MODULE step
   ...
   USE lib_fortran    ! MPP library nj
   USE lib_mpp        ! MPP library nj
   ...
   !! $Id: step.F90 6381 2016-03-10 14:12:49Z acc $
...
!!gm
      ...
      IF(lwp) WRITE(numout,*) "nj- step.f90-1: glob_sum(sshb)= ", ztmpnj, 'm'
      ztmpnj = glob_sum( sshn(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-1: glob_sum(sshn)= ", ztmpnj, 'm'
      ztmpnj = glob_sum( ssha(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-1: glob_sum(ssha)= ", ztmpnj, 'm'
      ...
      ztmpnj = glob_sum( sshb(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-2: glob_sum(sshb)= ", ztmpnj, 'm'
      ztmpnj = glob_sum( sshn(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-2: glob_sum(sshn)= ", ztmpnj, 'm'
      ztmpnj = glob_sum( ssha(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-2: glob_sum(ssha)= ", ztmpnj, 'm'
                            CALL wzv           ( kstp )  ! now cross-level velocity
      ztmpnj = glob_sum( sshb(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-3: glob_sum(sshb)= ", ztmpnj, 'm'
      ztmpnj = glob_sum( sshn(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-3: glob_sum(sshn)= ", ztmpnj, 'm'
      ztmpnj = glob_sum( ssha(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-3: glob_sum(ssha)= ", ztmpnj, 'm' 
!!gm : why also here ????
      IF( ln_sto_eos    )   CALL sto_pts( tsn )                             ! Random T/S fluctuations
!!gm
...
!!jc: fs simplification
      ...
      IF( ln_dynspg_ts ) THEN                         ! vertical scale factors and vertical velocity need to be updated
         ...
         IF(.NOT.ln_linssh) CALL dom_vvl_sf_nxt( kstp, kcall=2 )  ! after vertical scale factors (update depth average component)
                            CALL wzv        ( kstp )              ! now cross-level velocity 
      ztmpnj = glob_sum( sshb(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-4: glob_sum(sshb)= ", ztmpnj, 'm'
      ztmpnj = glob_sum( sshn(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-4: glob_sum(sshn)= ", ztmpnj, 'm'
      ztmpnj = glob_sum( ssha(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-4: glob_sum(ssha)= ", ztmpnj, 'm'
      ENDIF
...
!!jc2: dynnxt must be the latest call. e3t_b are indeed updated in that routine
      ...
      !
      ztmpnj = glob_sum( sshb(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-5: glob_sum(sshb)= ", ztmpnj, 'm'
      ztmpnj = glob_sum( sshn(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-5: glob_sum(sshn)= ", ztmpnj, 'm'
      ztmpnj = glob_sum( ssha(:,:) )
      IF(lwp) WRITE(numout,*) "nj- step.f90-5: glob_sum(ssha)= ", ztmpnj, 'm'
...
!! nj : move this before CALL rst_write
...
!!jc in fact update is useless at last time step, but do it for global diagnostics
      ...
      ENDIF
#endif
!!nj      IF( ln_diahsb  )   CALL dia_hsb( kstp )         ! - ML - global conservation diagnostics
