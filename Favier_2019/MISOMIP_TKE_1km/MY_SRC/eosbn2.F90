MODULE eosbn2
   ...
   ! reference temperature/salinity for simplified EOS (used for misomip+)
   ! REAL(wp) ::   rn_Tref = 10._wp
   ! REAL(wp) ::   rn_Sref = 35._wp  
   REAL(wp) ::   rn_Tref = -1._wp  !!misomip+
   REAL(wp) ::   rn_Sref = 34.2_wp !!misomip+
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: eosbn2.F90 6140 2015-12-21 11:35:23Z timgraham $
...
CONTAINS
   ...
   SUBROUTINE eos_insitu( pts, prd, pdep )
      ...
      CASE( 1 )                !==  simplified EOS  ==!
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  zt  = pts  (ji,jj,jk,jp_tem) - rn_Tref
                  zs  = pts  (ji,jj,jk,jp_sal) - rn_Sref
                  zh  = pdep (ji,jj,jk)
                  ztm = tmask(ji,jj,jk)
                  !
   ...
   SUBROUTINE eos_insitu_2d( pts, pdep, prd )
      ...
      CASE( 1 )                !==  simplified EOS  ==!
         ...
         DO jj = 1, jpjm1
            DO ji = 1, fs_jpim1   ! vector opt.
               !
               zt    = pts  (ji,jj,jp_tem)  - rn_Tref
               zs    = pts  (ji,jj,jp_sal)  - rn_Sref
   ...
   SUBROUTINE rab_3d( pts, pab )
      ...
      CASE( 1 )                  !==  simplified EOS  ==!
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  zt  = pts (ji,jj,jk,jp_tem) - rn_Tref   ! pot. temperature anomaly (t-T0)
                  zs  = pts (ji,jj,jk,jp_sal) - rn_Sref   ! abs. salinity anomaly (s-S0)
   ...
   SUBROUTINE rab_2d( pts, pdep, pab )
      ...
      CASE( 1 )                  !==  simplified EOS  ==!
         ...
         DO jj = 1, jpjm1
            DO ji = 1, fs_jpim1   ! vector opt.
               !
               zt    = pts  (ji,jj,jp_tem) - rn_Tref   ! pot. temperature anomaly (t-T0)
               zs    = pts  (ji,jj,jp_sal) - rn_Sref   ! abs. salinity anomaly (s-S0)
               zh    = pdep (ji,jj)                   ! depth at the partial step level
               !
               zn  = rn_a0 * ( 1._wp + rn_lambda1*zt + rn_mu1*zh ) + rn_nu*zs
   ...
   SUBROUTINE rab_0d( pts, pdep, pab )
      ...
      CASE( 1 )                  !==  simplified EOS  ==!
         !
         zt    = pts(jp_tem) - rn_Tref   ! pot. temperature anomaly (t-T0)
         zs    = pts(jp_sal) - rn_Sref   ! abs. salinity anomaly (s-S0)
   ...
   SUBROUTINE eos_pen( pts, pab_pe, ppen )
      ...
      CASE( 1 )                !==  Vallis (2006) simplified EOS  ==!
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  zt  = pts(ji,jj,jk,jp_tem) - rn_Tref  ! temperature anomaly (t-T0)
                  zs = pts (ji,jj,jk,jp_sal) - rn_Sref  ! abs. salinity anomaly (s-S0)
                  zh  = gdept_n(ji,jj,jk)              ! depth in meters  at t-point
                  ztm = tmask(ji,jj,jk)                ! tmask
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nameos in configuration namelist', lwp )
      ...
      !
      !!misomip+ rau0        = 1026._wp                 !: volumic mass of reference [kg/m3]
      !!misomip+ rcp         = 3991.86795711963_wp      !: heat capacity   [J/K]
      rau0        = 1027.51_wp      !!misomip+ : volumic mass of reference [kg/m3]
      rcp         = 3974._wp        !!misomip+ : heat capacity [J/K/kg]
      !
