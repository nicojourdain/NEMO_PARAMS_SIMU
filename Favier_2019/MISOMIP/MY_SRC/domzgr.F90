...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: domzgr.F90 6152 2015-12-21 22:33:57Z acc $
...
!!gm  bug ? :  must be a do loop with mj0,mj1
   ...
   SUBROUTINE zgr_isf
      ...
      END DO 
      IF ( cp_cfg .NE. "isomip" .AND. cp_cfg .NE. "ISOMIP" ) THEN
      ...
      IF ( cp_cfg .NE. "isomip" .AND. cp_cfg .NE. "ISOMIP" ) THEN
         WHERE (misfdep(:,:) <= 10 .AND. misfdep(:,:) .GT. 1)
            misfdep = 0; risfdep = 0.0_wp;
            mbathy  = 0; bathy   = 0.0_wp;
         END WHERE
         WHERE (bathy(:,:) <= 30.0 .AND. gphit < -60)
            misfdep = 0; risfdep = 0.0_wp;
            mbathy  = 0; bathy   = 0.0_wp;
         END WHERE
      END IF
...
!!nj-test      ! now fill single point in "coastline" of ice shelf, bathy, hole, and test again one cell tickness
!!nj-test      DO jl = 1,20
!!nj-test 
!!nj-test ! remove single point "bay" on isf coast line in the ice shelf draft'
!!nj-test         DO jk = 2, jpk
!!nj-test            WHERE (misfdep==0) misfdep=jpk
!!nj-test            zmask=0._wp
!!nj-test            WHERE (misfdep <= jk) zmask=1
!!nj-test            DO jj = 2, jpjm1
!!nj-test               DO ji = 2, jpim1
!!nj-test                  IF (misfdep(ji,jj) == jk) THEN
!!nj-test                     ibtest = zmask(ji-1,jj) + zmask(ji+1,jj) + zmask(ji,jj-1) + zmask(ji,jj+1)
!!nj-test                     IF (ibtest <= 1) THEN
!!nj-test                        risfdep(ji,jj)=gdepw_1d(jk+1) ; misfdep(ji,jj)=jk+1
!!nj-test                        IF (misfdep(ji,jj) > mbathy(ji,jj)) misfdep(ji,jj) = jpk
!!nj-test                     END IF
!!nj-test                  END IF
!!nj-test               END DO
!!nj-test            END DO
!!nj-test         END DO
!!nj-test         WHERE (misfdep==jpk)
!!nj-test             misfdep=0 ; risfdep=0._wp ; mbathy=0 ; bathy=0._wp
!!nj-test         END WHERE
!!nj-test         IF( lk_mpp ) THEN
!!nj-test            zbathy(:,:)  = FLOAT( misfdep(:,:) )
!!nj-test            CALL lbc_lnk( zbathy, 'T', 1. )
!!nj-test            misfdep(:,:) = INT( zbathy(:,:) )
!!nj-test
!!nj-test            CALL lbc_lnk( risfdep,'T', 1. )
!!nj-test            CALL lbc_lnk( bathy,  'T', 1. )
!!nj-test
!!nj-test            zbathy(:,:)  = FLOAT( mbathy(:,:) )
!!nj-test            CALL lbc_lnk( zbathy, 'T', 1. )
!!nj-test            mbathy(:,:)  = INT( zbathy(:,:) )
!!nj-test         ENDIF
!!nj-test 
!!nj-test ! remove single point "bay" on bathy coast line beneath an ice shelf'
!!nj-test         DO jk = jpk,1,-1
!!nj-test            zmask=0._wp
!!nj-test            WHERE (mbathy >= jk ) zmask=1
!!nj-test            DO jj = 2, jpjm1
!!nj-test               DO ji = 2, jpim1
!!nj-test                  IF (mbathy(ji,jj) == jk .AND. misfdep(ji,jj) >= 2) THEN
!!nj-test                     ibtest = zmask(ji-1,jj) + zmask(ji+1,jj) + zmask(ji,jj-1) + zmask(ji,jj+1)
!!nj-test                     IF (ibtest <= 1) THEN
!!nj-test                        bathy(ji,jj)=gdepw_1d(jk) ; mbathy(ji,jj)=jk-1
!!nj-test                        IF (misfdep(ji,jj) > mbathy(ji,jj)) mbathy(ji,jj) = 0
!!nj-test                     END IF
!!nj-test                  END IF
!!nj-test               END DO
!!nj-test            END DO
!!nj-test         END DO
!!nj-test         WHERE (mbathy==0)
!!nj-test             misfdep=0 ; risfdep=0._wp ; mbathy=0 ; bathy=0._wp
!!nj-test         END WHERE
!!nj-test         IF( lk_mpp ) THEN
!!nj-test            zbathy(:,:)  = FLOAT( misfdep(:,:) )
...
!!nj-test         ENDIF
!!nj-test 
!!nj-test ! fill hole in ice shelf
!!nj-test         zmisfdep = misfdep
!!nj-test         zrisfdep = risfdep
!!nj-test         WHERE (zmisfdep <= 1._wp) zmisfdep=jpk
!!nj-test         DO jj = 2, jpjm1
!!nj-test            DO ji = 2, jpim1
!!nj-test               ibtestim1 = zmisfdep(ji-1,jj  ) ; ibtestip1 = zmisfdep(ji+1,jj  )
!!nj-test               ibtestjm1 = zmisfdep(ji  ,jj-1) ; ibtestjp1 = zmisfdep(ji  ,jj+1)
!!nj-test               IF( zmisfdep(ji,jj) >= mbathy(ji-1,jj  ) ) ibtestim1 = jpk
!!nj-test               IF( zmisfdep(ji,jj) >= mbathy(ji+1,jj  ) ) ibtestip1 = jpk
!!nj-test               IF( zmisfdep(ji,jj) >= mbathy(ji  ,jj-1) ) ibtestjm1 = jpk
!!nj-test               IF( zmisfdep(ji,jj) >= mbathy(ji  ,jj+1) ) ibtestjp1 = jpk
!!nj-test               ibtest=MIN(ibtestim1, ibtestip1, ibtestjm1, ibtestjp1)
!!nj-test               IF( ibtest == jpk .AND. misfdep(ji,jj) >= 2) THEN
!!nj-test                  mbathy(ji,jj) = 0 ; bathy(ji,jj) = 0.0_wp ; misfdep(ji,jj) = 0 ; risfdep(ji,jj) = 0.0_wp
!!nj-test               END IF
!!nj-test               IF( zmisfdep(ji,jj) < ibtest .AND. misfdep(ji,jj) >= 2) THEN
!!nj-test                  misfdep(ji,jj) = ibtest
!!nj-test                  risfdep(ji,jj) = gdepw_1d(ibtest)
!!nj-test               ENDIF
!!nj-test            ENDDO
!!nj-test         ENDDO
!!nj-test 
...
!!nj-test         ENDIF 
!!nj-test !
!!nj-test !! fill hole in bathymetry
!!nj-test         zmbathy (:,:)=mbathy (:,:)
!!nj-test         DO jj = 2, jpjm1
!!nj-test            DO ji = 2, jpim1
!!nj-test               ibtestim1 = zmbathy(ji-1,jj  ) ; ibtestip1 = zmbathy(ji+1,jj  )
!!nj-test               ibtestjm1 = zmbathy(ji  ,jj-1) ; ibtestjp1 = zmbathy(ji  ,jj+1)
!!nj-test               IF( zmbathy(ji,jj) <  misfdep(ji-1,jj  ) ) ibtestim1 = 0
!!nj-test               IF( zmbathy(ji,jj) <  misfdep(ji+1,jj  ) ) ibtestip1 = 0
!!nj-test               IF( zmbathy(ji,jj) <  misfdep(ji  ,jj-1) ) ibtestjm1 = 0
!!nj-test               IF( zmbathy(ji,jj) <  misfdep(ji  ,jj+1) ) ibtestjp1 = 0
!!nj-test               ibtest=MAX(ibtestim1, ibtestip1, ibtestjm1, ibtestjp1)
!!nj-test               IF( ibtest == 0 .AND. misfdep(ji,jj) >= 2) THEN
!!nj-test                  mbathy(ji,jj) = 0 ; bathy(ji,jj) = 0.0_wp ; misfdep(ji,jj) = 0 ; risfdep(ji,jj) = 0.0_wp ;
!!nj-test               END IF
!!nj-test               IF( ibtest < zmbathy(ji,jj) .AND. misfdep(ji,jj) >= 2) THEN
!!nj-test                  mbathy(ji,jj) = ibtest
!!nj-test                  bathy(ji,jj)  = gdepw_1d(ibtest+1) 
!!nj-test               ENDIF
!!nj-test            END DO
!!nj-test         END DO
!!nj-test         IF( lk_mpp ) THEN 
...
!!nj-test            mbathy(:,:) = INT( zbathy(:,:) )
!!nj-test         ENDIF 
!!nj-test ! if not compatible after all check (ie U point water column less than 2 cells), mask U
!!nj-test         DO jj = 1, jpjm1
!!nj-test            DO ji = 1, jpim1
!!nj-test               IF (mbathy(ji,jj) == misfdep(ji+1,jj) .AND. mbathy(ji,jj) >= 1 .AND. mbathy(ji+1,jj) >= 1) THEN
!!nj-test                  mbathy(ji,jj)  = mbathy(ji,jj) - 1 ; bathy(ji,jj)   = gdepw_1d(mbathy(ji,jj)+1) ;
!!nj-test               END IF
!!nj-test            END DO
!!nj-test         END DO
!!nj-test         IF( lk_mpp ) THEN 
...
!!nj-test         DO jj = 1, jpjm1
!!nj-test            DO ji = 1, jpim1
!!nj-test               IF (misfdep(ji,jj) == mbathy(ji+1,jj) .AND. mbathy(ji,jj) >= 1 .AND. mbathy(ji+1,jj) >= 1) THEN
!!nj-test                  mbathy(ji+1,jj)  = mbathy(ji+1,jj) - 1;   bathy(ji+1,jj)   = gdepw_1d(mbathy(ji+1,jj)+1) ;
!!nj-test               END IF
!!nj-test            END DO
!!nj-test         END DO
!!nj-test         IF( lk_mpp ) THEN 
...
!!nj-test            mbathy(:,:) = INT( zbathy(:,:) )
!!nj-test         ENDIF 
!!nj-test ! if not compatible after all check (ie V point water column less than 2 cells), mask V
!!nj-test         DO jj = 1, jpjm1
!!nj-test            DO ji = 1, jpi
!!nj-test               IF (mbathy(ji,jj) == misfdep(ji,jj+1) .AND. mbathy(ji,jj) >= 1 .AND. mbathy(ji,jj+1) >= 1) THEN
!!nj-test                  mbathy(ji,jj)  = mbathy(ji,jj) - 1 ; bathy(ji,jj)   = gdepw_1d(mbathy(ji,jj)+1) ;
!!nj-test               END IF
!!nj-test            END DO
!!nj-test         END DO
...
!!nj-test         DO jj = 1, jpjm1
!!nj-test            DO ji = 1, jpi
!!nj-test               IF (misfdep(ji,jj) == mbathy(ji,jj+1) .AND. mbathy(ji,jj) >= 1 .AND. mbathy(ji,jj+1) >= 1) THEN
!!nj-test                  mbathy(ji,jj+1)  = mbathy(ji,jj+1) - 1 ; bathy(ji,jj+1) = gdepw_1d(mbathy(ji,jj+1)+1) ;
!!nj-test               END IF
!!nj-test            END DO
!!nj-test         END DO
!!nj-test         IF( lk_mpp ) THEN 
...
!!nj-test            mbathy(:,:) = INT( zbathy(:,:) )
!!nj-test         ENDIF 
!!nj-test ! if not compatible after all check, mask T
!!nj-test         DO jj = 1, jpj
!!nj-test            DO ji = 1, jpi
!!nj-test               IF (mbathy(ji,jj) <= misfdep(ji,jj)) THEN
!!nj-test                  misfdep(ji,jj) = 0 ; risfdep(ji,jj) = 0._wp ; mbathy(ji,jj)  = 0 ; bathy(ji,jj)   = 0._wp ;
!!nj-test               END IF
!!nj-test            END DO
!!nj-test         END DO
!!nj-test 
!!nj-test         WHERE (mbathy(:,:) == 1)
!!nj-test            mbathy = 0; bathy = 0.0_wp ; misfdep = 0 ; risfdep = 0.0_wp
!!nj-test         END WHERE
!!nj-test      END DO 
!!nj-test! end check compatibility ice shelf/bathy
