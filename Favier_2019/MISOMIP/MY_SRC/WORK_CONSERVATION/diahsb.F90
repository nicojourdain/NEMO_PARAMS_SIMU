MODULE diahsb
   ...
   REAL(wp), PUBLIC, SAVE ::   hsb_surf_tot              ! ocean surface
   REAL(wp), PUBLIC, SAVE ::   frc_t, frc_s, frc_v   ! global forcing trends
   REAL(wp), PUBLIC, SAVE ::   frc_wn_t, frc_wn_s    ! global forcing trends
   !
   REAL(wp), DIMENSION(:,:)  , ALLOCATABLE ::   hsb_surf 
   REAL(wp), DIMENSION(:,:)  , ALLOCATABLE ::   hsb_surf_ini      , ssh_ini          !
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: diahsb.F90 6140 2015-12-21 11:35:23Z timgraham $
...
CONTAINS
   ...
   SUBROUTINE dia_hsb( kt )
      ...
      ! ------------------------- !
      z_frc_trd_v = r1_rau0 * glob_sum( - ( emp(:,:) - rnf(:,:) + fwfisf(:,:) ) * hsb_surf(:,:) ) ! volume fluxes
      z_frc_trd_t =           glob_sum( sbc_tsc(:,:,jp_tem) * hsb_surf(:,:) )                               ! heat fluxes
      z_frc_trd_s =           glob_sum( sbc_tsc(:,:,jp_sal) * hsb_surf(:,:) )                               ! salt fluxes
      ! Add runoff    heat & salt input
      IF( ln_rnf    )   z_frc_trd_t = z_frc_trd_t + glob_sum( rnf_tsc(:,:,jp_tem) * hsb_surf(:,:) )
      IF( ln_rnf_sal)   z_frc_trd_s = z_frc_trd_s + glob_sum( rnf_tsc(:,:,jp_sal) * hsb_surf(:,:) )
      ! Add ice shelf heat & salt input
      IF( ln_isf    ) z_frc_trd_t = z_frc_trd_t + glob_sum( risf_tsc(:,:,jp_tem) * hsb_surf(:,:) )
      ! Add penetrative solar radiation
      IF( ln_traqsr )   z_frc_trd_t = z_frc_trd_t + r1_rau0_rcp * glob_sum( qsr     (:,:) * hsb_surf(:,:) )
      ! Add geothermal heat flux
      IF( ln_trabbc )   z_frc_trd_t = z_frc_trd_t +               glob_sum( qgh_trd0(:,:) * hsb_surf(:,:) )
      !
      IF( ln_linssh ) THEN
         IF( ln_isfcav ) THEN
            DO ji=1,jpi
               DO jj=1,jpj
                  z2d0(ji,jj) = hsb_surf(ji,jj) * wn(ji,jj,mikt(ji,jj)) * tsb(ji,jj,mikt(ji,jj),jp_tem)
                  z2d1(ji,jj) = hsb_surf(ji,jj) * wn(ji,jj,mikt(ji,jj)) * tsb(ji,jj,mikt(ji,jj),jp_sal)
               END DO
            END DO
         ELSE
            z2d0(:,:) = hsb_surf(:,:) * wn(:,:,1) * tsb(:,:,1,jp_tem)
            z2d1(:,:) = hsb_surf(:,:) * wn(:,:,1) * tsb(:,:,1,jp_sal)
         END IF
      ...
      zdiff_v1 = glob_sum_full( hsb_surf(:,:) * sshn(:,:) - hsb_surf_ini(:,:) * ssh_ini(:,:) )
      ...
      IF( ln_linssh ) THEN
         IF( ln_isfcav ) THEN
            DO ji = 1, jpi
               DO jj = 1, jpj
                  z2d0(ji,jj) = hsb_surf(ji,jj) * ( tsn(ji,jj,mikt(ji,jj),jp_tem) * sshn(ji,jj) - ssh_hc_loc_ini(ji,jj) ) 
                  z2d1(ji,jj) = hsb_surf(ji,jj) * ( tsn(ji,jj,mikt(ji,jj),jp_sal) * sshn(ji,jj) - ssh_sc_loc_ini(ji,jj) ) 
               END DO
            END DO
         ELSE
            z2d0(:,:) = hsb_surf(:,:) * ( tsn(:,:,1,jp_tem) * sshn(:,:) - ssh_hc_loc_ini(:,:) ) 
            z2d1(:,:) = hsb_surf(:,:) * ( tsn(:,:,1,jp_sal) * sshn(:,:) - ssh_sc_loc_ini(:,:) ) 
         END IF
      ...
      DO jk = 1, jpkm1
         ...
         zdiff_v2 = zdiff_v2 + glob_sum_full( hsb_surf(:,:) * tmask(:,:,jk)            &
            &                           * e3t_n(:,:,jk) - hsb_surf_ini(:,:) * e3t_ini(:,:,jk) )
         ! heat content variation
         zdiff_hc = zdiff_hc + glob_sum_full(  hsb_surf(:,:) * tmask(:,:,jk)                                   & 
            &                           * e3t_n(:,:,jk) * tsn(:,:,jk,jp_tem) - hsb_surf_ini(:,:) * hc_loc_ini(:,:,jk) ) 
         ! salt content variation
         zdiff_sc = zdiff_sc + glob_sum_full( hsb_surf    (:,:) * tmask(:,:,jk)                           &
                                        * e3t_n(:,:,jk) * tsn(:,:,jk,jp_sal) - hsb_surf_ini(:,:) * sc_loc_ini(:,:,jk) )
      ENDDO
      ...
      DO jk = 1, jpkm1
         zvol_tot  = zvol_tot + glob_sum_full( hsb_surf(:,:) * tmask(:,:,jk) * e3t_n(:,:,jk) )
      END DO

!!gm to be added ?
!      IF( ln_linssh ) THEN            ! fixed volume, add the ssh contribution
!        zvol_tot = zvol_tot + glob_sum( hsb_surf(:,:) * sshn(:,:) )
!      ENDIF
!!gm end
   ...
   SUBROUTINE dia_hsb_rst( kt, cdrw )
     ...
     IF( TRIM(cdrw) == 'READ' ) THEN        ! Read/initialise 
        IF( ln_rstart ) THEN                   !* Read the restart file
           ...
           ENDIF
           CALL iom_get( numror, jpdom_autoglo, 'hsb_surf_ini', hsb_surf_ini ) ! ice sheet coupling
       ...
       ELSE
          ...
          IF(lwp) WRITE(numout,*) '~~~~~~~'
          hsb_surf_ini(:,:) = e1e2t(:,:) * tmask_i(:,:)         ! initial ocean surface
     ...
     ELSEIF( TRIM(cdrw) == 'WRITE' ) THEN   ! Create restart file
        ...
        ENDIF
        CALL iom_rstput( kt, nitrst, numrow, 'hsb_surf_ini', hsb_surf_ini )      ! ice sheet coupling
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namhsb in configuration namelist', lwp )
      ...
      ! 1 - Allocate memory !
      ! ------------------- !
      ALLOCATE( hc_loc_ini(jpi,jpj,jpk), sc_loc_ini(jpi,jpj,jpk), hsb_surf_ini(jpi,jpj), &
         &      e3t_ini(jpi,jpj,jpk), hsb_surf(jpi,jpj),  ssh_ini(jpi,jpj), STAT=ierror  )
      IF( ierror > 0 ) THEN
      ...
      IF(lwp) WRITE(numout,*) '~~~~~~~'
      hsb_surf(:,:) = e1t(:,:) * e2t(:,:) * tmask_i(:,:)      ! masked surface grid cell area
      hsb_surf_tot  = glob_sum( hsb_surf(:,:) )               ! total ocean surface area
