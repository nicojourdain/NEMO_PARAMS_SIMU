...
!TOM> no horizontal drag for sea ice! real, PARAMETER :: pp_Cd_ih=0.0012 ! (lateral) Drag coeff. between bergs and sea-ice
   ...
   LOGICAL , PUBLIC ::   ln_restart_boxes   = .false.   !: Recompute the origin of each iceberg after restart
...
#endif
         &      ff_e(0:jpi+1,0:jpj+1) , fr_e(0:jpi+1,0:jpj+1), tt_e(0:jpi+1,0:jpj+1), ssh_e(0:jpi+1,0:jpj+1) ,   hicth(0:jpi+1,0:jpj+1),&
