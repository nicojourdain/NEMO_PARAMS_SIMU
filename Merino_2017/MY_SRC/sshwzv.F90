MODULE sshwzv   
   ...
   USE timing          ! Timing
   USE lib_fortran
!  USE sbcfwb     ! PM add it but likely to be useless
...
CONTAINS
   ...
   SUBROUTINE ssh_wzv( kt ) 
      ...
      REAL(wp) ::   zavessh, zarea      ! temporary scalar
      !!----------------------------------------------------------------------
      !
      ...
      ENDIF
      zarea   = glob_sum( e1e2t(:,:) )
      zavessh = glob_sum( e1e2t(:,:) * sshn(:,:) ) / zarea
      IF (lwp) PRINT *, 'kt = ', kt, ' mean sshn = ',zavessh
      !
