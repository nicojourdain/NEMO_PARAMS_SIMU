MODULE sbcmod
   ...
   USE sbcisf           ! surface boundary condition: ice shelf formulation (ISF)
...
CONTAINS
   ...
   SUBROUTINE sbc_init
      ...
      NAMELIST/namsbc/ nn_fsbc   , ln_ana , ln_flx  , ln_blk_clio, ln_blk_core, ln_cpl,   &
         ...
         &             ln_ssr    , nn_fwb    , ln_cdgw , ln_wave , ln_sdw, nn_isf
      !!----------------------------------------------------------------------
      ...
      IF( nn_isf == 0 ) THEN                      ! no specific treatment ice shelf cavity runoff in depth
      ...
      ENDIF
      !
!{  DRAKKAR  bug fix : missing call in reference version 3.5 beta (corrected May,3 2014 ! )
...
!}
   ...
   SUBROUTINE sbc( kt )
      ...
      IF( nn_isf   /= 0  )   CALL sbc_isf( kt )                   ! compute ice shelf contribution 
