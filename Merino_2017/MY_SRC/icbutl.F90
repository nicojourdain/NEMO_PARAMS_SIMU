...
CONTAINS
   ...
   SUBROUTINE icb_utl_copy()
      ...
      CALL lbc_lnk_e( fr_e, 'T', +1._wp, 1, 1 )
      CALL lbc_lnk_e( tt_e, 'T', +1._wp, 1, 1 )
      CALL lbc_lnk_e(hicth, 'T', +1._wp, 1, 1 )  
...
# endif
   ...
   SUBROUTINE icb_utl_velocityIntegral(kLevels, pi, puo, pj, pvo )
      ...
      DO iter=1,kLevels
          ...
          puo = puo + pu*e3t(mi1(pi),mj1(pj),iter)
          pvo = pvo + pv*e3t(mi1(pi),mj1(pj),iter)
          metric = metric + e3t(mi1(pi),mj1(pj),iter)
!print *, 'level', iter, 'pu', pu, 'metric',metric
   ...
   SUBROUTINE icb_utl_boyantConvectionMelt(kLevels, pi, pj, z1_rday, mvo )
      ...
      DO iter=1,kLevels
          ...
          mvo = mvo + mv*e3t(mi1(pi),mj1(pj),mlevel)
          metric = metric + e3t(mi1(pi),mj1(pj),mlevel)
      END DO
...
!MODIF
 !if(ii.eq.0) ii=1
   ...
   REAL(wp) FUNCTION icb_utl_bilin_e( pet, peu, pev, pef, pi, pj )
      ...
      zj = pj - REAL(ij,wp)
!print *,'MENSAJE: ', 'narea',narea,'nimpp',nimpp, pj, pi, ii,ij
      ! find position in this processor
