...
!CDIR NOVERRCHK
   ...
   FUNCTION tfreez( psal, ppress ) RESULT( ptf )
      !!----------------------------------------------------------------------
      !!                 ***  ROUTINE eos_init  ***
      !!
      ...
      REAL(wp), DIMENSION(jpi,jpj), INTENT(in   ), OPTIONAL ::   ppress   ! pressure             [dBar]
      ...
      REAL(wp), DIMENSION(jpi,jpj)                ::   zpress ! pressure [dBar]
      !!----------------------------------------------------------------------
      !
      IF( PRESENT(ppress) ) THEN 
         zpress(:,:)=ppress(:,:) 
      ELSE 
         zpress(:,:)=0.0 
      ENDIF 
   ...
   FUNCTION tfreez1D( psal, ppress ) RESULT( ptf )
      !!----------------------------------------------------------------------
      !!                 ***  ROUTINE eos_init  ***
      !!
      !! ** Purpose :   Compute the sea surface freezing temperature [Celcius]
      ...
      REAL(wp), INTENT(in   ), OPTIONAL ::   ppress   ! pressure             [dBar]
      ! Leave result array automatic rather than making explicitly allocated
      REAL(wp)                ::   ptf    ! freezing temperature [Celcius]
      REAL(wp)                ::   zpress ! pressure [dBar]
      !!----------------------------------------------------------------------
      !
      IF( PRESENT(ppress) ) THEN
         zpress=ppress
