MODULE sbcssr
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: distcoast2   ! use to read the distance and then for weight purpose
   ...
   SUBROUTINE sbc_ssr_init
      ...
      IF( nn_sssr >= 1 ) THEN      !* set sf_sss structure & allocate arrays
         ...
         IF ( ln_sssr_msk ) THEN
            ...
            ALLOCATE( distcoast2(jpi,jpj),STAT=ierror )  
            IF( ierror > 0 )   CALL ctl_stop( 'STOP', 'sbc_ssr: unable to allocate erp and qrp array' )
            ...
            CALL iom_get  ( inum, jpdom_data, sn_coast%clvar, distcoast2 ) ! read tcoast  in m
            CALL iom_close( inum )
