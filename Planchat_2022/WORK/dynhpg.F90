...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: dynhpg.F90 5921 2015-11-25 17:13:47Z mathiot $
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namdyn_hpg in configuration namelist', lwp )
      ...
      IF (       ln_isfcav ) THEN
         ...
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               END DO
               IF (ikt .GE. 2) ziceload(ji,jj) = ziceload(ji,jj) + (2._wp * znad + zrhdtop_isf(ji,jj) + zrhd(ji,jj,ikt-1)) &
   ...
   SUBROUTINE hpg_isf( kt )
      ...
      REAL(wp), POINTER, DIMENSION(:,:)     ::  zrhdtop_oce, zpshpi, zpshpj
      !!----------------------------------------------------------------------
      !
      CALL wrk_alloc( jpi,jpj,  2, ztstop) 
      CALL wrk_alloc( jpi,jpj,jpk, zhpi, zhpj)
      CALL wrk_alloc( jpi,jpj,     zrhdtop_oce, zpshpi, zpshpj) 
      !
      ! Local constant initialization
      zcoef0 = - grav * 0.5_wp
...
!==================================================================================
      ...
      CALL wrk_dealloc( jpi,jpj,2  , ztstop)
      CALL wrk_dealloc( jpi,jpj,jpk, zhpi, zhpj)
      CALL wrk_dealloc( jpi,jpj    , zrhdtop_oce, zpshpi, zpshpj)
      !
