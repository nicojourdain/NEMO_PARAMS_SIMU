...
!!gm  bug ? :  must be a do loop with mj0,mj1
   ...
   SUBROUTINE zgr_isf
      ...
      REAL(wp) :: rn_isfhw_deep = 1.e-3 , rn_isfhw_shallow = 1.0e-3 ! threshold to define grounding line in deep/shallow water
      ...
      NAMELIST/namzgr_isf/nn_kisfmax, rn_zisfmax, &
              &           rn_isfdep_min, rn_isfhw_deep, rn_isfhw_shallow, rn_isfshallow, &
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namzgr_isf in configuration namelist', lwp )
      ...
      IF (lwp) THEN
         ...
         WRITE(numout,*) ' rn_isfhw_deep    = ',rn_isfhw_deep        !
         WRITE(numout,*) ' rn_isfhw_shallow = ',rn_isfhw_shallow     !
         WRITE(numout,*) ' rn_isfshallow    = ',rn_isfshallow        !
      ...
      !
      ! 1.0 set iceshelf to the minimum depth allowed
      WHERE(risfdep(:,:) > 0.0_wp .AND. risfdep(:,:) < MAX(rn_isfdep_min,e3t_1d(1)))
         risfdep(:,:)=rn_isfdep_min
      END WHERE
      !  
      ! 1.1 ground ice shelf if water column less than X m
      WHERE( bathy(:,:) - risfdep(:,:) < rn_isfhw_deep .AND. (bathy(:,:) > rn_isfshallow) ) 
         risfdep(:,:)=0.0 ; misfdep(:,:) = 1
         bathy  (:,:)=0.0 ; mbathy (:,:) = 0
      END WHERE
      !
      ! 1.2 ensure a minimum thickness for iceshelf cavity in shallow water
      WHERE( (bathy(:,:) <= rn_isfshallow) .AND. (bathy(:,:) - risfdep(:,:) < rn_isfhw_shallow) ) 
         risfdep(:,:)=0.0 ; misfdep(:,:) = 1
         bathy  (:,:)=0.0 ; mbathy (:,:) = 0
      END WHERE

      ! nico :
      WHERE(risfdep(:,:) .EQ. rn_isfdep_min)
         risfdep(:,:) = 0.0 
      END WHERE
      ...
      END DO 
      !
      ! 2.1 be sure misfdep not below mbathy 
      ! warning because of condition 4 we could have 'wet cell with misfdep below mbathy
      ! risfdep of these cells will be fix later on (see 5)
      WHERE( misfdep > mbathy .AND. mbathy > 0 ) misfdep=mbathy
      !
