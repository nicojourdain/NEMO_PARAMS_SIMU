...
CONTAINS
   SUBROUTINE FillPool3D(kiseed, kjseed, kkseed, rdta, rfill)
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk)                :: rseedmap, rseedmap_b  !
 
      INTEGER :: ip=0                 ! size of the pile
      INTEGER :: ii  , ij  , ik   , kii, kjj, jj, kk    ! working integer
      INTEGER :: iip1, ijp1, ikp1
      INTEGER :: iim1, ijm1, ikm1
      INTEGER :: nseed
      ...
      IF ((mi0(ii) == mi1(ii)) .AND. (mj0(ij) == mj1(ij))) THEN   ! T if seed on the local domain
         ...
         END IF
      END IF
      nseed=SUM(rseedmap); IF( lk_mpp )   CALL mpp_sum( nseed )  ! nseed =0 means on land => WARNING later on
      !
      ...
      IF (nseed > 0) THEN
         ...
         DO WHILE ( nseed /= 0 )
            ...
            !
            ! exchange seed
            nseed=SUM(rseedmap); IF( lk_mpp )   CALL mpp_sum( nseed )  ! this is the sum of all the point check this iteration
            !
            rseedmap_b(:,:,:)=rseedmap(:,:,:)
            CALL lbc_lnk(rseedmap,'T',1.)
            !
   ...
   SUBROUTINE FillPool2D(kiseed, kjseed, rdta, rfill)
      ...
      REAL(wp), DIMENSION(jpi,jpj) :: rseedmap, rseedmap_b 
      ...
      IF (nseed > 0) THEN
         ...
         DO WHILE ( nseed .NE. 0 )
            DO WHILE ( ASSOCIATED(seed) )
               ip=ip+1
               ii=seed%i; ij=seed%j ; rseedmap(ii,ij)=1.
               ! update bathy and update pile size
               CALL del_idx(seed) 
               ! 
               ! check neighbour cells
            ...
            DO ii=1,jpi
               DO jj=1,jpj
                  IF (rseedmap(ii,jj) > 0.0 .AND. rseedmap(ii,jj) /= rseedmap_b(ii,jj) .AND. rdta(ii,jj) > 0 .AND. rdta(ii,jj) /= rfill) THEN
   ...
   !
   SUBROUTINE create_idx(pt_idx, ki, kj, kk)
      TYPE (idx), POINTER :: pt_idx
      INTEGER, INTENT(in) :: ki, kj, kk
      !
      ! initialised all field to NULL()
      NULLIFY(pt_idx)
      !
      ! allocate new element
      ALLOCATE(pt_idx)
      pt_idx%i=ki ; pt_idx%j=kj ; pt_idx%k=kk ;
      pt_idx%next => NULL()  
   END SUBROUTINE create_idx
