MODULE diaharm 
   ...
   USE sbcisf          ! nj, to do harmonic analysis on utbl, vtnl
   ...
   !! $Id: diaharm.F90 5956 2015-11-30 19:55:41Z mathiot $
            ...
            DO jc = 1, 2
               ...
               DO jj = 1,jpj
                  DO ji = 1,jpi
                     ...
                     !nj ana_temp(ji,jj,nhc,2) = ana_temp(ji,jj,nhc,2) + ztemp*un_b(ji,jj)*ssumask(ji,jj)
                     ana_temp(ji,jj,nhc,2) = ana_temp(ji,jj,nhc,2) + ztemp*utbl(ji,jj)
                     !nj ana_temp(ji,jj,nhc,3) = ana_temp(ji,jj,nhc,3) + ztemp*vn_b(ji,jj)*ssvmask(ji,jj)
                     ana_temp(ji,jj,nhc,3) = ana_temp(ji,jj,nhc,3) + ztemp*vtbl(ji,jj)
                  END DO
               END DO
               !
            END DO
