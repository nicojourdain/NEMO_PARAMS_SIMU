...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: ldfslp.F90 5200 2015-04-07 14:22:54Z mathiot $
   ...
   SUBROUTINE ldf_slp( kt, prd, pn2 )
      ...
      IF ( ln_traldf_iso .OR. ln_dynldf_iso ) THEN 
         ...
         DO jk = 2, jpkm1
            ...
            !                                !          tmask(ik+1) /= 0   =>   all pn2  /= 0   =>   zdzr divides by 2
         ...
         IF ( ln_isfcav ) THEN
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  zhmlpu(ji,jj) = MAX(hmlpt(ji,jj), hmlpt(ji+1,jj  ), 5._wp) - 0.5 * ( risfdep(ji,jj) + risfdep(ji+1,jj  ) )
                  zhmlpv(ji,jj) = MAX(hmlpt(ji,jj), hmlpt(ji  ,jj+1), 5._wp) - 0.5 * ( risfdep(ji,jj) + risfdep(ji  ,jj+1) )
               END DO
            END DO
         ELSE
            DO jj = 2, jpjm1
            ...
            END DO
         END IF
! PM + GM can be optimized by using : zuslp_hmlpu(ji,jj)= uslpml(ji,jj) / zhmlpu (ji,jj)
   ...
   SUBROUTINE ldf_slp_mxl( prd, pn2, p_gru, p_grv, p_dzr )
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1
            ...
            iku = MIN(  MAX( nmln(ji,jj) , nmln(ji+1,jj) ) , jpkm1  )   ! ML (MAX of T-pts, bound by jpkm1)
            ikv = MIN(  MAX( nmln(ji,jj) , nmln(ji,jj+1) ) , jpkm1  )   !
