...
902      IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_isf in configuration namelist', lwp )
  ...
  END SUBROUTINE sbc_isf_bg03
   ...
   SUBROUTINE sbc_isf_gammats(gt, gs, zqhisf, zqwisf )
      ...
      REAL(wp), DIMENSION(:,:), POINTER :: zustar, zutide ! friction velocity, tidal velocity
      ...
      INTEGER  :: iutide       ! File handle
      !!---------------------------------------------------------------------
      CALL wrk_alloc( jpi,jpj, zustar, zutide )
      !
      ...
      !! nico's modification to account for tides effects under ice shelves :
         CALL iom_open ( 'tidal_velocity.nc', iutide)
         CALL iom_get  ( iutide, jpdom_data, 'utide', zutide)
         CALL iom_close( iutide )

      !! compute ustar (eq. 24)
         zustar(:,:) = SQRT( rn_tfri2 * (utbl(:,:) * utbl(:,:) + vtbl(:,:) * vtbl(:,:) + zutide(:,:) * zutide(:,:) + rn_tfeb2) )
      ...
      !! compute gammats
         ...
         CALL lbc_lnk(gs(:,:),'T',1.)
      END IF
      CALL wrk_dealloc( jpi,jpj, zustar, zutide )
