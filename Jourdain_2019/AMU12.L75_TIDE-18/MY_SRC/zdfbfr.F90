...
#  include "domzgr_substitute.h90"
   ...
   !! $Id: zdfbfr.F90 5621 2015-07-21 11:25:36Z mathiot $
...
CONTAINS
   ...
   SUBROUTINE zdf_bfr( kt )
      ...
      REAL(wp) ::   zvu, zuv, ztmp               ! temporary scalars
      REAL(wp), POINTER, DIMENSION(:,:) ::  zbfrt, ztfrt
      REAL(wp), POINTER, DIMENSION(:,:) :: zecu, zecv
      !!----------------------------------------------------------------------
      !
      ...
      IF( nn_bfr == 2 ) THEN                 ! quadratic bottom friction only
         ...
         CALL wrk_alloc( jpi, jpj, zecu, zecv )
...
! JC: possible WAD implementation should modify line below if layers vanish
         ...
         DO jj = 2, jpjm1
            DO ji = 2, jpim1
               ...
               !
               zecu(ji,jj) = SQRT(  un(ji,jj,ikbu) * un(ji,jj,ikbu) + zvu*zvu + rn_bfeb2  )
               zecv(ji,jj) = SQRT(  vn(ji,jj,ikbv) * vn(ji,jj,ikbv) + zuv*zuv + rn_bfeb2  )
               !
               bfrua(ji,jj) = - 0.5_wp * ( zbfrt(ji,jj) + zbfrt(ji+1,jj  ) ) * zecu(ji,jj)
               bfrva(ji,jj) = - 0.5_wp * ( zbfrt(ji,jj) + zbfrt(ji  ,jj+1) ) * zecv(ji,jj)
               !
         ...
         CALL iom_put('velu_bbl',zecu(:,:))
         CALL iom_put('velv_bbl',zecv(:,:))
         ...
         IF ( ln_isfcav ) THEN
            ...
            DO jj = 2, jpjm1
               DO ji = 2, jpim1
                  ...
                  zuv  = 0.25 * (  un(ji,jj  ,ikbv) + un(ji-1,jj  ,ikbv)     &
                     &           + un(ji,jj+1,ikbv) + un(ji-1,jj+1,ikbv)  )
              !
                  zecu(ji,jj) = SQRT(  un(ji,jj,ikbu) * un(ji,jj,ikbu) + zvu*zvu + rn_tfeb2 )
                  zecv(ji,jj) = SQRT(  vn(ji,jj,ikbv) * vn(ji,jj,ikbv) + zuv*zuv + rn_tfeb2 )
              !
                  tfrua(ji,jj) = - 0.5_wp * ( ztfrt(ji,jj) + ztfrt(ji+1,jj  ) ) * zecu(ji,jj) * (1._wp - umask(ji,jj,1))
                  tfrva(ji,jj) = - 0.5_wp * ( ztfrt(ji,jj) + ztfrt(ji  ,jj+1) ) * zecv(ji,jj) * (1._wp - vmask(ji,jj,1))
              ! (ISF) END ====================================================================
              ! in case of 2 cell water column, we assume each cell feels the top and bottom friction
                  IF ( miku(ji,jj) + 1 .GE. mbku(ji,jj) ) THEN
            ...
            CALL iom_put('velu_tbl',zecu(:,:))
            CALL iom_put('velv_tbl',zecv(:,:))
         END IF
         !
         !
         ...
         CALL wrk_dealloc( jpi,jpj,zecu,zecv )
      ENDIF
      !
